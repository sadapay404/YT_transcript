...
export function LandingView() {
  return (
    <main className="mx-auto w-full max-w-[1400px] flex-1 px-4 pt-10 pb-20 sm:px-6">
      {/* ── Hero ─────────────────────────────────────────────────────────── */}
      <section className="flex flex-col items-center text-center">
        ...
        {/* URL bar preview — real fetching arrives in Step 2. */}
        <motion.div
          initial={{ opacity: 0, y: 14 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.55, type: "spring", stiffness: 260, damping: 28 }}
          className="panel mt-8 flex w-full max-w-2xl items-center gap-2 p-2"
        >
          <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-ink/[0.06] text-ink-faint">
            <Link2 className="h-4 w-4" />
          </span>
          <span className="flex-1 truncate text-left text-[13px] text-ink-faint">
            https://www.youtube.com/watch?v=…
          </span>
          <span className="chip hidden sm:inline-flex">Step 2</span>
          <span className="btn btn-primary pointer-events-none opacity-70">
            Extract
        ...
        <p className="mt-3 text-[11.5px] text-ink-faint">
          Step 1 shipped: app shell, token system, four themes, typography
          studio, motion primitives. The fetcher lands in Step 2.
        </p>
      </section>
